...
def draw_values():
    ...
    files = glob.glob('./files_to_plot/*.txt')
    ...
    fig.update_yaxes(title_text="flops / cycle", secondary_y=False)
    ...
    for file in files:
        ...
        values = np.array(flops).astype('float64')
    
        opt_name = '_'.join(file.split('/')[-1].split('.')[0].split('_')[1:])

        if opt_name not in opts:
            opts[opt_name] = dict()
            opts[opt_name][file.split('/')[-1].split('.')[0].split('_')[0]] = values
        else:
            opts[opt_name][file.split('/')[-1].split('.')[0].split('_')[0]] = values

    for opt_name in opts.keys():
        print(opt_name)
        flops_per_cycle = opts[opt_name]['flops'] / opts[opt_name]['cycles']
        plot_values(sizes, flops_per_cycle, beautiful_names[opt_name])
    
    fig.add_trace(go.Scatter(x=sizes, y=4 * np.ones(len(sizes)), name='Scalar peak'), secondary_y=False)
    # fig.add_trace(go.Scatter(x=sizes, y=16 * np.ones(len(sizes)), line=dict(color='red'), name='Vectorized peak'), secondary_y=False)
    fig.show()
