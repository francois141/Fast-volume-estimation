...
const string FILE_CYCLES_PATH = "./benchmark_infrastructure/cycles_cubes_original.txt";
// const string FILE_CYCLES_PATH = "./benchmark_infrastructure/cycles_simplex_original.txt";
const string FILE_SIZES_PATH = "./benchmark_infrastructure/size_cubes_original.txt";
// const string FILE_SIZES_PATH = "./benchmark_infrastructure/size_simplex_original.txt";
...
long long int measure_cycles(string example_name) {
    ...
    vol::polytope_orig p(m, n);
    p.msg_off = true;
    for (int i = 0; i < m; i++) {
        p.vecb(vecb[i], i);

        for (int j = 0; j < n; j++) {
            p.matA(matA[i * n + j], i, j);
        }
    }
