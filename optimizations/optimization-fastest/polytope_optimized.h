...
inline double rand_double(double upper_bound);
inline int rand_int(int upper_bound);
...
void init_polytope(polytope **pp, size_t m, size_t n) {
    ...
    p->m = m;
    p->n = n;
    p->A = (double *)aligned_alloc(
        32, ROUND_UP(m, 4) * ROUND_UP(n, 4) * sizeof(double));
    memset(p->A, 0, m * n * sizeof(double));
    p->b = (double *)aligned_alloc(32, ROUND_UP(m, 4) * sizeof(double));
    memset(p->b, 0, ROUND_UP(m, 4) * sizeof(double));
    p->debug_msg = true;
    ...
    p->ball_radiuses = (double *)aligned_alloc(
        32, ROUND_UP(p->dimension_limit * sizeof(double), 32));
    ...
    p->sampling_points_start =
        (double *)aligned_alloc(32, ROUND_UP(n * sizeof(double), 32));
    memset(p->sampling_points_start, 0, n * sizeof(double));
...
void cleanup(polytope *p) {
    ...
    free(p->bounds);
    free(p->A_transpose_padded);
    free(p->A_transpose_padded_inv);
    free(p->mask_greater);
    free(p->mask_smaller);
    free(p);
    return;
...
void check_halfplanes(polytope *p) {
    ...
    double *A_aux =
        (double *)aligned_alloc(32, m_rounded * n_rounded * sizeof(double));
    for (int i = 0; i < m_rounded; ++i) {
        for (int j = 0; j < n_rounded; j += 4) {
            _mm256_store_pd(A_aux + i * n_rounded + j,
                            _mm256_load_pd(p->A + i * n_rounded + j));
        }
    }
    free(p->A);
    p->A = A_aux;
    double *b_aux = (double *)aligned_alloc(32, m_rounded * sizeof(double));
    for (int i = 0; i < m_rounded; i += 4) {
        _mm256_store_pd(b_aux + i, _mm256_load_pd(p->b + i));
    }
    free(p->b);
    p->b = b_aux;
...
void preprocess(polytope *p) {
    ...
    */
    ...
    for (int i = 0; i < n_rounded; i++) {
        for (int j = 0; j < m_rounded; j++) {
            if (i >= n || j >= m) {
                ...
                p->A_transpose_padded_inv[i * m_rounded + j] = 1.0;
            } else {
                p->A_transpose_padded[i * m_rounded + j] = p->A[j * n + i];
                p->A_transpose_padded_inv[i * m_rounded + j] =
                    1.0 / p->A[j * n + i];
            }
        }
    }
    ...
    for (int i = 0; i < n_rounded; i++) {
        for (int j = 0; j < m_rounded; j += 4) {
            __m256d A_tmp =
                _mm256_load_pd(p->A_transpose_padded + i * m_rounded + j);
            _mm256_store_pd(p->mask_greater + i * m_rounded + j,
                            _mm256_cmp_pd(A_tmp, zero_vector, _CMP_GT_OQ));
            _mm256_store_pd(p->mask_smaller + i * m_rounded + j,
                            _mm256_cmp_pd(A_tmp, zero_vector, _CMP_LT_OQ));
        }
    }
    ...
    p->bounds = (double *)aligned_alloc(32, ROUND_UP(m, 4) * sizeof(double));
...
double estimate_volume_with_rand_functions(polytope *p, int coef,
    ...
    double *alpha = (double *)aligned_alloc(
        32, ROUND_UP(p->dimension_limit * sizeof(double), 32));
    memset(alpha, 0, p->dimension_limit * sizeof(double));
    long long *volK = (long long *)aligned_alloc(
        32, ROUND_UP(p->dimension_limit * sizeof(long long), 32));
    memset(volK, 0, p->dimension_limit * sizeof(long long));

    __m256d pow_two_inv_n_inv_vector = _mm256_set1_pd(p->pow_two_inv_n_inv);
    for (k = p->dimension_limit - 2; k >= 0; k--) {
        for (i = volK[k + 1]; i < step_size; i++) {
            double m_walk =
                walk_with_rand_functions(p, k, rand_double, rand_int);
        ...
        if (volK[k] < step_size) {
...

            {
                ...
                for (i1 = 0; i1 < m_rounded; i1 += 4) {
                    ...
                    __m256d b_vector = _mm256_load_pd(p->b + i1);

                    __m256d tmp1 = _mm256_sub_pd(b_vector, bounds_vector);
                    bounds_vector = _mm256_fnmadd_pd(
                        tmp1, pow_two_inv_n_inv_vector, b_vector);

                    _mm256_store_pd(p->bounds + i1, bounds_vector);
                }
            }

            {
                ...
                for (i1 = 0; i1 < n_rounded; i1 += 4) {
                    __m256d sampling_points_vector =
                        _mm256_load_pd(p->sampling_points_start + i1);
                    sampling_points_vector = _mm256_mul_pd(
                        sampling_points_vector, pow_two_inv_n_inv_vector);

                    sampling_poinst_sq_sum_vector = _mm256_fmadd_pd(
                        sampling_points_vector, sampling_points_vector,
                        sampling_poinst_sq_sum_vector);

                    _mm256_store_pd(p->sampling_points_start + i1,
                                    sampling_points_vector);
                }
                double sq_sum[4];
                _mm256_store_pd(&sq_sum[0], sampling_poinst_sq_sum_vector);
                p->sampling_poinst_sq_sum =
                    sq_sum[0] + sq_sum[1] + sq_sum[2] + sq_sum[3];
            }
...
double walk_with_rand_functions(polytope *p, int k,
    ...
    __m256d bound_max_vector_2 = bound_max_vector;
    __m256d bound_min_vector_2 = bound_min_vector;

    __m256d bound_max_vector_3 = bound_max_vector;
    __m256d bound_min_vector_3 = bound_min_vector;

    __m256d bound_max_vector_4 = bound_max_vector;
    __m256d bound_min_vector_4 = bound_min_vector;
    int i = 0;
    // 16
    for (; i + 16 < m_rounded; i += 16) {
        // Loads
        __m256d bounds_vector = _mm256_load_pd(p->bounds + i);
        __m256d A_transpose_vector_inv = _mm256_load_pd(
            p->A_transpose_padded_inv + random_direction * m_rounded + i);

        // 4
        __m256d bounds_vector_2 = _mm256_load_pd(p->bounds + i + 4);
        __m256d A_transpose_vector_inv_2 = _mm256_load_pd(
            p->A_transpose_padded_inv + random_direction * m_rounded + i + 4);

        // 8

        __m256d bounds_vector_3 = _mm256_load_pd(p->bounds + i + 8);
        __m256d A_transpose_vector_inv_3 = _mm256_load_pd(
            p->A_transpose_padded_inv + random_direction * m_rounded + i + 8);

        // 12
        __m256d bounds_vector_4 = _mm256_load_pd(p->bounds + i + 12);
        __m256d A_transpose_vector_inv_4 = _mm256_load_pd(
            p->A_transpose_padded_inv + random_direction * m_rounded + i + 12);

        // Prod vector
        __m256d prod_vector =
            _mm256_mul_pd(bounds_vector, A_transpose_vector_inv);
        __m256d prod_vector_2 =
            _mm256_mul_pd(bounds_vector_2, A_transpose_vector_inv_2);
        __m256d prod_vector_3 =
            _mm256_mul_pd(bounds_vector_3, A_transpose_vector_inv_3);
        __m256d prod_vector_4 =
            _mm256_mul_pd(bounds_vector_4, A_transpose_vector_inv_4);

        // Masks
        __m256d mask_greater =
            _mm256_load_pd(p->mask_greater + random_direction * m_rounded + i);
        __m256d mask_smaller =
            _mm256_load_pd(p->mask_smaller + random_direction * m_rounded + i);

        // 4
        __m256d mask_greater_2 = _mm256_load_pd(
            p->mask_greater + random_direction * m_rounded + i + 4);
        __m256d mask_smaller_2 = _mm256_load_pd(
            p->mask_smaller + random_direction * m_rounded + i + 4);

        // 8
        __m256d mask_greater_3 = _mm256_load_pd(
            p->mask_greater + random_direction * m_rounded + i + 8);
        __m256d mask_smaller_3 = _mm256_load_pd(
            p->mask_smaller + random_direction * m_rounded + i + 8);

        // 12
        __m256d mask_greater_4 = _mm256_load_pd(
            p->mask_greater + random_direction * m_rounded + i + 12);
        __m256d mask_smaller_4 = _mm256_load_pd(
            p->mask_smaller + random_direction * m_rounded + i + 12);
        ...
        bound_max_vector =
            _mm256_blendv_pd(bound_max_vector, min_vector, mask_greater);
        bound_min_vector =
            _mm256_blendv_pd(bound_min_vector, max_vector, mask_smaller);
        // 4
        __m256d min_vector_2 = _mm256_min_pd(bound_max_vector_2, prod_vector_2);
        __m256d max_vector_2 = _mm256_max_pd(bound_min_vector_2, prod_vector_2);

        bound_max_vector_2 =
            _mm256_blendv_pd(bound_max_vector_2, min_vector_2, mask_greater_2);
        bound_min_vector_2 =
            _mm256_blendv_pd(bound_min_vector_2, max_vector_2, mask_smaller_2);
        // 8
        __m256d min_vector_3 = _mm256_min_pd(bound_max_vector_3, prod_vector_3);
        __m256d max_vector_3 = _mm256_max_pd(bound_min_vector_3, prod_vector_3);
        bound_max_vector_3 =
            _mm256_blendv_pd(bound_max_vector_3, min_vector_3, mask_greater_3);
        bound_min_vector_3 =
            _mm256_blendv_pd(bound_min_vector_3, max_vector_3, mask_smaller_3);
        // 12
        __m256d min_vector_4 = _mm256_min_pd(bound_max_vector_4, prod_vector_4);
        __m256d max_vector_4 = _mm256_max_pd(bound_min_vector_4, prod_vector_4);
        bound_max_vector_4 =
            _mm256_blendv_pd(bound_max_vector_4, min_vector_4, mask_greater_4);
        bound_min_vector_4 =
            _mm256_blendv_pd(bound_min_vector_4, max_vector_4, mask_smaller_4);
    }
    bound_max_vector = _mm256_min_pd(bound_max_vector, bound_max_vector_2);
    bound_max_vector_3 = _mm256_min_pd(bound_max_vector_3, bound_max_vector_4);
    bound_max_vector = _mm256_min_pd(bound_max_vector, bound_max_vector_3);

    bound_min_vector = _mm256_max_pd(bound_min_vector, bound_min_vector_2);
    bound_min_vector_3 = _mm256_max_pd(bound_min_vector_3, bound_min_vector_4);
    bound_min_vector = _mm256_max_pd(bound_min_vector, bound_min_vector_3);
    ...
    for (int i = 0; i < m_rounded; i += 4) {
        ...
        // m_rounded + i];
        __m256d bounds_vector = _mm256_load_pd(p->bounds + i);
        __m256d A_transpose_vector = _mm256_load_pd(
...
inline double rand_double(double upper_bound) {
    ...
    double adjusted = u.d - 1.0;
    return __DBL_MIN__ + adjusted * (upper_bound - __DBL_MIN__);
}

inline int rand_int(int upper_bound) {
    double adjusted = (next() >> 11) * 0x1.0p-53;
    return (0 + adjusted * (upper_bound - 0.1));
