...
void preprocess(polytope *p) {
    ...
    */
    ...
    p->bounds = (double *)calloc(m, sizeof(double));
...
double estimate_volume_with_rand_functions(polytope *p, int coef,
    ...
    for (k = p->dimension_limit - 2; k >= 0; k--) {
        ...
        if (volK[k] < step_size) {
            ...
            matrix_divide_constant_inplace(1, n, p->sampling_points_start,
                                           pow((double)2, (double)1.0 / n));
        } else {
            alpha[k] = 1;
        }
    }
