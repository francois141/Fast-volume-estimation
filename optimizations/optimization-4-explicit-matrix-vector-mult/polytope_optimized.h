...
void preprocess(polytope *p) {
    ...
    double *temp1 = (double *)malloc(sizeof(double) * p->n * 1);
    double *temp2;
    while (++counter > 0) {
        int i;
        ...
        for (int k = 0; k < p->m; k++) {
            double temp = 0.0;
            for (int j = 0; j < p->n; j++) {
                temp += p->A[k * p->n + j] * origin_arr[j];
            }
            distance[k] = p->b[k] - temp;
        }
        ...
        for (i = 0; i < p->m; i++) {
            if (distance[i] < 0) {
                ...
                for (int j = 0; j < p->n ; j++) {
                    double temp2 = 0.0;
                    for (int k = 0; k < p->n; k++) {
                        temp2 += p->A[i * p->n + k] * T[k * p->n + j];
                    }
                    temp += temp2 * p->A[i * p->n + j];
                }
                tm[i] = temp;
    ...
    */
    ...
    p->uball_volume = uball_volume(p->n);
    ...
    for (k = p->dimension_limit - 2; k >= 0; k--) {
        for (i = volK[k + 1]; i < step_size; i++) {
            ...
            if (m < p->ball_radiuses[0]) {
                ...
                volK[(int)trunc(p->n * log(m) * p->two_log_two_inv) + 1]++;
            }
        }
        for (i = 0; i < k; i++) {
            volK[k] += volK[i];
...
double walk_with_rand_functions(polytope *p, int k,
    ...
    for (int i = 0; i < p->m; ++i) {
        ...
        p->bounds[i] = p->b_dimensions[random_direction][i] - prod;
    }
    for (int i = 0; i < p->m; ++i) {
        if (p->A[i * p->n + random_direction] > 0) {
            if (p->bounds[i] < bound_max) {
                bound_max = p->bounds[i];
            }
        } else if (p->A[i * p->n + random_direction] < 0) {
            if (p->bounds[i] > bound_min) {
                bound_min = p->bounds[i];
            }
        }
    }
