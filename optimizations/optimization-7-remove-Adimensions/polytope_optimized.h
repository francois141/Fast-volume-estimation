...
void preprocess(polytope *p) {
    ...
    */
    ...
    p->A_prod_sampling_points = (double*) calloc(m, sizeof(double));
...
double estimate_volume_with_rand_functions(polytope *p, int coef,
    ...
    for (k = p->dimension_limit - 2; k >= 0; k--) {
        ...
        for (int i1 = 0; i1 < m; i1++){
            p->A_prod_sampling_points[i1] = 0;
            for (int i2 = 0; i2 < n; i2++){
                p->A_prod_sampling_points[i1] += p->A[i1 * n + i2] * p->sampling_points_start[i2];
            }
        }
