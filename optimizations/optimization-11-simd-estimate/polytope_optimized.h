...
void init_polytope(polytope **pp, size_t m, size_t n) {
    ...
    p->m = m;
    p->n = n;
    p->A = (double *)aligned_alloc(32, ROUND_UP(m * n * sizeof(double), 32));
    memset(p->A, 0, m * n * sizeof(double));
    p->b = (double *)aligned_alloc(32, ROUND_UP(m * sizeof(double), 32));
    memset(p->b, 0, m * sizeof(double));
    p->debug_msg = true;
    ...
    p->ball_radiuses = (double *)aligned_alloc(32, ROUND_UP(p->dimension_limit * sizeof(double), 32));
    ...
    p->sampling_points_start = (double *)aligned_alloc(32, ROUND_UP(n * sizeof(double), 32));
    memset(p->sampling_points_start, 0, n * sizeof(double));

    // Vector and matrix for each dimension
    p->A_dimensions = (double **)aligned_alloc(32, ROUND_UP(n * sizeof(double *), 32));
    memset(p->A_dimensions, 0, n * sizeof(double *));
    p->b_dimensions = (double **)aligned_alloc(32, ROUND_UP(n * sizeof(double *), 32));
    memset(p->b_dimensions, 0, n * sizeof(double *));
    ...
    p->determinant = 0;
    for (int i = 0; i < n; ++i) {
        p->A_dimensions[i] = (double *)aligned_alloc(32, ROUND_UP(m * n * sizeof(double), 32));
        memset(p->A_dimensions[i], 0, m * n * sizeof(double));
        p->b_dimensions[i] = (double *)aligned_alloc(32, ROUND_UP(m * sizeof(double), 32));
        memset(p->b_dimensions[i], 0, m * sizeof(double));
    }
...
void cleanup(polytope *p) {
    ...
    if (p->A_transpose_padded != NULL) {
        free(p->A_transpose_padded);
    }
    if (p->A_transpose_inv_padded != NULL) {
        free(p->A_transpose_inv_padded);
    }
...
void preprocess(polytope *p) {
    ...
    */
    ...
    p->A_transpose_padded = (double*)aligned_alloc(32, ROUND_UP(sizeof(double) * m_rounded * n_rounded, 32));
    p->A_transpose_inv_padded = (double*)aligned_alloc(32, ROUND_UP(sizeof(double) * m_rounded * n_rounded, 32));
    ...
    for(int i = 0; i < n_rounded;i++) {
        for(int j = 0; j < m_rounded;j++) {
            ...
            p->A_transpose_inv_padded[i*m_rounded+j] = 1.0/p->A_transpose_padded[i*m_rounded+j];
        }
    }
    ...
    p->bounds = (double *)aligned_alloc(32, ROUND_UP(ROUND_UP(m,4) * sizeof(double), 32));
    memset(p->bounds, 0, ROUND_UP(m, 4) * sizeof(double));
...
double estimate_volume_with_rand_functions(polytope *p, int coef,
    ...
    double *alpha = (double *)aligned_alloc(32, ROUND_UP(p->dimension_limit * sizeof(double), 32));
    memset(alpha, 0, p->dimension_limit * sizeof(double));
    long long *volK = (long long *)aligned_alloc(32, ROUND_UP(p->dimension_limit * sizeof(long long), 32));
    ...
    for (k = p->dimension_limit - 2; k >= 0; k--) {
        for (i = volK[k + 1]; i < step_size; i++) {
            double m_walk = walk_with_rand_functions(p, k, rand_double, rand_int);
...

        {
            ...
            // VAR. no simd, original code
            for (int i = 0; i < k; i++) {
                volK[k] += volK[i];
            }

            // VAR. simd => performance slightly worse
            // __m256i volK_sum = _mm256_set1_epi64x(0);
            // int i = 0;

            // for (i = 0; i < k-3; i += 4) {
            //     __m256i volK_vector = _mm256_load_si256((__m256i const *) (volK + i));
            //     volK_sum = _mm256_add_epi64(volK_sum, volK_vector);
            // }

            // long long volK_tmp[4];
            // _mm256_storeu_si256((__m256i *) &volK_tmp[0], volK_sum);
            // volK[k] += volK_tmp[0] + volK_tmp[1] + volK_tmp[2] + volK_tmp[3];
            ...
            // VAR. no simd, unrooled, different accumulators => performance is the same
...
double walk_with_rand_functions(polytope *p, int k,
    ...
    for (int i = 0; i < m_rounded; i += 4) {
        ...
        __m256d A_transpose_vector = _mm256_load_pd(p->A_transpose_padded + random_direction * m_rounded + i);
        __m256d A_transpose_inv_vector = _mm256_load_pd(p->A_transpose_inv_padded + random_direction * m_rounded + i);

        // double prod = p->bounds[i] / p->A_transpose_padded[random_direction * m_rounded + i];
        __m256d prod_vector = _mm256_mul_pd(bounds_vector, A_transpose_inv_vector);
