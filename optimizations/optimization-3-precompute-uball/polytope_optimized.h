...
void check_halfplanes(polytope *p) {
    ...
    // load constraints
    int *ind = (int *)malloc(sizeof(int) * (p->n + 1));
    double *val = (double *)malloc(sizeof(double) * (p->n + 1));

    for (int i = 1; i < p->m + 1; i++) {
        for (int j = 1; j < p->n + 1; j++) {
            ind[j] = j;
            val[j] = p->A[(i - 1) * p->n + j - 1];
        }
        glp_set_mat_row(lp, i, p->n, ind, val);
    ...
    for (int i = 1; i < p->m + 1;) {
        ...
        if (glp_get_status(lp) == GLP_NOFEAS) {
            ...
            // A.shed_row(i - 1);
            remove_row(p->m, p->n, p->A, i - 1);
            // b.shed_row(i - 1);
            remove_cell(p->m, p->b, i - 1);
            p->m--;
        } else {
            glp_set_row_bnds(lp, i, GLP_UP, 0, p->b[i - 1]);
            i++;
    ...
    p->A = (double *)realloc(p->A, p->m * p->n * sizeof(double));
    p->b = (double *)realloc(p->b, p->m * sizeof(double));
...
bool generate_init_ellipsoid(polytope *p, double *r2, double *origin_arr) {
    *r2 = 0;
    memset(origin_arr, 0, p->n * sizeof(double));
    ...
    // load constraints
    int *ind = (int *)malloc((p->n + 1) * sizeof(int));
    double *val = (double *)malloc((p->n + 1) * sizeof(double));
...
void preprocess(polytope *p) {
    ...
    double c1 = (2 * pow(p->n, 2) + pow(1 - p->n / p->beta_r, 2)) *
                (1 - 1.0 / pow(p->beta_r, 2)) / (2 * pow(p->n, 2) - 2);
    double c2 = (1 - p->n / p->beta_r) / (p->n + 1);
    double c3 = p->beta_r * p->beta_r;
    double c4 = 2 * c2 / (1 - 1.0 / p->beta_r);

    // Init ellipsoid (R2I, 0), Transformation = R2I, origin = 0.
    double *origin_arr = (double *)malloc(p->n * sizeof(double));
    double *r2 = (double *)malloc(sizeof(double));

    generate_init_ellipsoid(p, r2, origin_arr);

    double *T = (double *)calloc(p->n * p->n, sizeof(double));

    for (int i = 0; i < p->n; i++) {
        T[i * p->n + i] = *r2;
    }

    double *distance = (double *)calloc(p->m, sizeof(double));

    double *tm = (double *)calloc(p->m, sizeof(double));
    ...
    while (++counter > 0) {
        ...
        double *temp1 = (double *)malloc(sizeof(double) * p->m);

        matrix_multiplication_wrapper(p->m, 1, p->n, 1, 0, p->A, origin_arr,
                                      temp1);
        matrix_matrix_sub(p->m, 1, p->b, temp1, distance);
        ...
        for (i = 0; i < p->m; i++) {
            if (distance[i] < 0) {
                ...
                temp1 = (double *)malloc(sizeof(double) * p->n * 1);
                matrix_multiplication_wrapper(1, p->n, p->n, 1, 0,
                                              (p->A + p->n * i), T, temp1);

                temp2 = (double *)malloc(sizeof(double) * 1 * 1);
                matrix_multiplication_wrapper(1, 1, p->n, 1, 0, temp1,
                                              (p->A + p->n * i), temp2);
        ...
        temp1 = (double *)malloc(sizeof(double) * p->n * 1);
        temp2 = (double *)malloc(sizeof(double) * p->n * 1);

        // vec t = T * A.row(i).t() / sqrt(tm(i));      temp1 = t
        matrix_multiplication_wrapper(p->n, 1, p->n, 1, 0, T, (p->A + p->n * i),
                                      temp1);
        // matrix_multiply_constant_inplace(p->n, 1, temp1, 1.0 / sqrt(tm[i]));
        matrix_divide_constant_inplace(p->n, 1, temp1, sqrt(tm[i]));

        // ori = ori - t * c2;      temp1 = t
        matrix_multiply_constant(p->n, 1, temp1, c2, temp2);
        matrix_matrix_sub_inplace(p->n, 1, origin_arr, temp2);

        free(temp2);
        temp2 = (double *)malloc(sizeof(double) * p->n * p->n * 1);
    ...
    double *Trans = (double *)malloc(p->n * p->n * sizeof(double));
    memcpy(Trans, T, p->n * p->n * sizeof(double));

    int return_code = cholesky_decomposition(p->n, Trans);
    assert(return_code == 0);

    // cout << Trans << endl;

    // b = beta_r * (b - A * ori);
    temp1 = (double *)malloc(p->m * 1 * sizeof(double));
    matrix_multiplication_wrapper(p->m, 1, p->n, 1, 0, p->A, origin_arr, temp1);
    matrix_matrix_sub_inplace(p->m, 1, p->b, temp1);
    matrix_multiply_constant_inplace(p->m, 1, p->b, p->beta_r);
    free(temp1);

    // A = A * Trans.t();
    // TODO, change the flags passed to cblas instead of actually computing the
    // transpose
    temp1 = (double *)malloc(p->n * p->n * sizeof(double));
    temp2 = (double *)malloc(p->m * p->n * sizeof(double));

    matrix_transpose(p->n, p->n, Trans, temp1);
    matrix_multiplication_wrapper(p->m, p->n, p->n, 1, 0, p->A, temp1, temp2);
    ...
    double *all_ones = (double *)malloc(1 * p->n * sizeof(double));  // 1 x N
    for (int i = 0; i < p->n; ++i) {
        all_ones[i] = 1;
    }
    ...
    for (int i = 0; i < p->n; i++) {
        double *A_divider =
                (double *)malloc(p->m * 1 * sizeof(double));  // M x 1 (col vector)

        for (int j = 0; j < p->m; ++j) {
            A_divider[j] = p->A[j * p->n + i];
        }
        ...
        double *A_divider_matrix =
                (double *)malloc(p->m * p->n * sizeof(double));  // M x N
        matrix_multiplication_wrapper(p->m, p->n, 1, 1, 0, A_divider, all_ones,
                                      A_divider_matrix);

        for (int j = 0; j < p->m; ++j) {
            for (int k = 0; k < p->n; ++k) {
                // if (isfinite(p->A[j * p->n + k]) &&
                // isfinite(A_divider_matrix[j * p->n + k])){
                //     if (fabs(p->A[j * p->n + k]) < DOUBLE_EPS &&
                //     fabs(A_divider_matrix[j * p->n + k]) < DOUBLE_EPS)
                //         p->A_dimensions[i][j * p->n + k] = NAN;
                //     else if (fabs(A_divider_matrix[j * p->n + k]) <
                //     DOUBLE_EPS)
                //         p->A_dimensions[i][j * p->n + k] = INFINITY;
                //     else
                p->A_dimensions[i][j * p->n + k] =
                        p->A[j * p->n + k] / A_divider_matrix[j * p->n + k];
                // }
                // else
                //     p->A_dimensions[i][j * p->n + k] = NAN;
            }
    ...
    p->determinant = 1.0;
    for (int i = 0; i < p->n; i++) {
        p->determinant *= Trans[i * p->n + i];
    }

    p->determinant /= pow(p->beta_r, p->n);

    // Custom preprocess to be faste
    p->uball_precomputed = uball_volume(p->n);
...
double estimate_volume_with_rand_functions(polytope *p, int coef,
    ...
    p->volume = p->uball_precomputed * p->determinant;
...
void read_polytope(polytope** p, FILE *fin) {
    ...
    for (int i = 0; i < input->m; i++) {
        for (int j = 0; j < input->n; j++) {
            nb_returned_arguments = fscanf(fin, "%lf", &input->A[i * input->n + j]);
