    ...
    */
    ...
    for (int i = 0; i < n_rounded; i++) {
        for (int j = 0; j < m_rounded; j += 4) {
            ...
            _mm256_store_pd(p->mask1 + i * m_rounded + j,
                            _mm256_cmp_pd(A_tmp, zero_vector, _CMP_GT_OQ));
            _mm256_store_pd(p->mask2 + i * m_rounded + j,
...
double walk_with_rand_functions(polytope *p, int k,
    ...
    for (int i = 0; i < m_rounded; i += 4) {
        ...
        __m256d mask1 =
            _mm256_load_pd(p->mask1 + random_direction * m_rounded + i);
        __m256d mask2 =
            _mm256_load_pd(p->mask2 + random_direction * m_rounded + i);
